...
class NetworkRequestObserver(appContext: AppContext, filter: NetworkRequestFilter? = null) :
  ...
  // the monitor's fan-out (`shouldEmit`) and a write from `setFilter` are atomic: a `setFilter`
  // call never leaves a request observed under a half-applied filter. `@Volatile` alone would make
  // each field read atomic, but the lock keeps the read and the swap a single indivisible step.
  ...
  override fun shouldEmit(url: String, method: String): Boolean = synchronized(filterLock) {
