    ...
    ModuleDefinition {
      ...
      OnCreate {
        ...
        sessionCoordinator = SessionCoordinator(
          ...
          startTimestamp = appSessionStartTimestamp,
          metadata = metadata
        )
      ...
      Class("Session", SessionSharedObject::class) {
        ...
        AsyncFunction("addMetric") Coroutine { ref: SessionSharedObject, metric: SessionMetricInput ->
          // TODO(@ubax): This assumes that we only have a main session which can get metrics
          // Refactor the code so that joins happen within session object
          sessionCoordinator.awaitSessionReady()
