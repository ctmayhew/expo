...
struct NetworkRequestObserverFilterTests {
  @Test
  func `shouldEmit reflects the active filter`() {
    // The suite is `@AppMetricsActor`-isolated, so the actor-isolated `filter` is set and read
    // here synchronously — exercising `shouldEmit`'s logic without racing the `Task` hop that the
    ...
    #expect(observer.shouldEmit(url: URL(string: "https://expo.dev/x")!, method: "POST"))
    #expect(!observer.shouldEmit(url: URL(string: "https://expo.dev/x")!, method: "GET"))
...
/** Collecting delegate that only accepts requests for a single host, exercising the monitor's
 per-delegate `shouldEmit` consult at the fan-out site. */
private final class FilteringDelegate: NetworkRequestObserverDelegate, @unchecked Sendable {
  ...
  func shouldEmit(url: URL, method: String) -> Bool {
